...
const MODLAR = [
  ...
  { slug: "kacis_odasi", ad: "Kaçış Odası" },
  { slug: "sanal_lab", ad: "Sanal Lab" },
  { slug: "duel", ad: "Düello" },
];

const MADALYA = ["🥇", "🥈", "🥉"];

export default function LiderlikBoard() {
    ...
    <main className="mx-auto max-w-3xl px-6 py-12">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-slate-400">En iyi skorlar burada listelenir.</p>
        </div>
        <Link
          href="/harita"
      ...
      <div className="mt-6 overflow-hidden rounded-3xl border border-slate-800">
        {loading ? (
          ...
          <p className="p-10 text-center text-slate-500">Henüz skor yok. İlk sen oyna!</p>
        ) : (
          rows.map((r, i) => (
            <div
              key={r.userId}
            ...
            >
              <div className="flex items-center gap-4">
                <span className="w-10 text-center text-2xl font-black">{MADALYA[i] ?? i + 1}</span>
                <div>
